...
$(document).ready(function(){
  ...
  .done(function(dataObj){
    ...
    dataObj.map(function(todo){
      ...
      $('ul').append(
        `<li data-id=${todo._id} ${completed}>${todo.description}<span><i class='far fa-trash-alt'></i></span></li>`
      );
    })
  })
...
$('ul').append(
  `<li data-id=${newTodo._id}${completed}>${newTodo.description}<span><i class='far fa-trash-alt'></i></span></li>`
);
 $('input').val("");
})
.fail(function(err){
